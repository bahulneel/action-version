...
const parser = __nccwpck_require__(40)
const regex = __nccwpck_require__(568)
...
var conventional_commits_parser = __nccwpck_require__(375);
...
async function readJSON(file) {
  return JSON.parse(await promises_namespaceObject.readFile(file, 'utf8'));
}

async function writeJSON(file, data) {
  await promises_namespaceObject.writeFile(file, JSON.stringify(data, null, 2) + '\n', 'utf8');
}
...
function parseCommits(log) {
  ...
  for (const entry of log.split(/^commit /gm).slice(1)) {
    ...
    const parsed = conventional_commits_parser.sync(message);
  ...
  if (rootPkg.workspaces) {
    ...
    for (const pattern of patterns) {
      for (const match of globSync(pattern, { cwd: process.cwd(), absolute: true })) {
        ...
        try {
          await promises_namespaceObject.access(external_path_namespaceObject.join(match, 'package.json'));
          dirs.add(external_path_namespaceObject.resolve(match));
        } catch { }
      }
    }
...
async function buildDepGraph(pkgDirs) {
  ...
  for (const dir of pkgDirs) {
    const pkg = await readJSON(external_path_namespaceObject.join(dir, 'package.json'));
...
async function getCommitsAffecting(dir, sinceTag) {
  ...
  const log = (0,external_child_process_namespaceObject.execSync)(`git log ${range} --pretty=medium -- ${dir}`, { encoding: 'utf8' });
...
async function getLastTag(pkgName) {
  ...
  try {
    const tags = (0,external_child_process_namespaceObject.execSync)('git tag', { encoding: 'utf8' })
      .split('\n')
      .filter(Boolean)
...
async function commitAndPush(dir, msg) {
  await git.add([external_path_namespaceObject.join(dir, 'package.json')]);
...
async function runTest(dir, packageManager) {
  try {
    const result = (0,external_child_process_namespaceObject.spawnSync)(packageManager, ['test'], { cwd: dir, stdio: 'inherit' });
...
async function main() {
  try {
    ...
    const rootPkg = await readJSON(__nccwpck_require__.ab + "package.json");
    const packageManager = (await promises_namespaceObject.stat(external_path_namespaceObject.join(rootDir, 'yarn.lock')).catch(() => false)) ? 'yarn' : 'npm';
    ...
    for (const name of order) {
      ...
      await writeJSON(external_path_namespaceObject.join(dir, 'package.json'), pkg);
    ...
    if (rootPkg.workspaces) {
      ...
      if (rootBump !== 'patch') {
        ...
        await writeJSON(__nccwpck_require__.ab + "package.json", rootPkg);
